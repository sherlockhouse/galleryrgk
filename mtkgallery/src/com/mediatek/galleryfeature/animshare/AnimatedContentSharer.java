...
public class AnimatedContentSharer {
    ...
    private static final Object mReShareLock = new Object();
    private static final String KEY_SHARE_AS_GIF = "key_share_as_gif";
    private static final int SHARE_TYPE_VIDEO = 1;
    private static final int SHARE_TYPE_GIF = 2;
    ...
    private final List<IShareHooker> mShareHookers = new ArrayList<IShareHooker>();
    ...
    public interface IMediaDataGetter {
        ...
        /**
         * Redirect current media.
         * @param uri                item uri
         * @param fromActivityResult need result
         */
    ...
    public interface IShareHooker {
        public boolean share(final Activity activity, final Intent intent,
                final IMediaDataGetter mediaDataGetter);
    }

    /**
     * create AnimatedContentSharer.
     * @param activity         activity
     * @param mediaCenter      media center
     * @param mediaDataUpdator mediaDataUpdator
     */
    public AnimatedContentSharer(Activity activity, MediaCenter mediaCenter,
            IMediaDataGetter mediaDataUpdator) {
        MtkLog.i(TAG, "<new>");
        mActivity = activity;
        ...
        mHandler = new Handler(mActivity.getMainLooper()) {
            ...
            public void handleMessage(Message msg) {
                ...
                case MSG_SHARE_AS_VIDEO:
                    MtkLog.i(TAG, "<handleMessage> MSG_SHARE_AS_VIDEO, onShareAsVideoRequested");
    ...
    public void onResume() {
        ...
        if (!mIsInGenerating) {
            ...
            // The outstanding prerequisite: the return value of getVideoSharableImage
            // is last media data.
        ...
        synchronized (mReShareLock) {
            mReShareLock.notifyAll();
        }
    }
    ...
    /**
     * onShareTargetSelected callback.
     * @param intent intent
    ...
    public void registerShareHooker(final IShareHooker shareHooker) {
        mShareHookers.add(shareHooker);
    }

    private boolean isShareTypeChangeSupported(final Intent intent, final String mimeType) {
        ComponentName userSelectedComponent = intent.getComponent();
        Intent intentForChangedShareType = new Intent(intent);
        intentForChangedShareType.setComponent(null);
        intentForChangedShareType.setType(mimeType);
        List<ResolveInfo> resolveInfos = mActivity.getPackageManager().queryIntentActivities(
                intentForChangedShareType, 0);

        ComponentName candidateComponentForChangedShareType;
        ...
        for (int i = 0; i < resolveInfoCount; i++) {
            ...
            if (ActivityManager.checkComponentPermission(activityInfo.permission,
                ...
                candidateComponentForChangedShareType = new ComponentName(activityInfo.packageName,
                        activityInfo.name);
                if (userSelectedComponent.equals(candidateComponentForChangedShareType)) {
                    MtkLog.d(TAG, "<isShareTypeChangeSupported> return true: " + activityInfo.name);
                    return true;
                }
            }
        }

        MtkLog.d(TAG, "<isShareTypeChangeSupported> return false: " + userSelectedComponent);
        return false;
    }
        ...
        new Thread() {
            public void run() {
                ...
                if (mShareUri != null) {
                    ...
                    MtkLog.i(TAG, "<Thread-onShareAsVideoRequested> Wake up,"
                            + " send MSG_SHARE_AS_VIDEO");
    ...
    private void tryToShareAsVideo(Intent intent) {
        ...
        for (IShareHooker shareHooker : mShareHookers) {
            if (shareHooker.share(mActivity, intent, mMediaDataGetter)) {
                return;
            }
        }
        ...
        final String[] shareFormats = getShareFormatStrings(intent, item);
                ...
                if ((whichItemSelect >= 0) && (whichItemSelect < shareFormats.length)) {
                    ...
                    if (mShareType == SHARE_TYPE_VIDEO || mShareType == SHARE_TYPE_GIF) {
                        if (mShareType == SHARE_TYPE_GIF
                                || !isShareTypeChangeSupported(finalIntent, MIME_TYPE_VIDEO)) {
    ...
    private String[] getExportFormatStrings(MediaData mediaData) {
        return new String[] { mActivity.getString(R.string.m_share_format_image),
                mActivity.getString(R.string.m_share_format_video) };
    }

    private String[] getShareFormatStrings(final Intent intent, final MediaData mediaData) {
        ArrayList<String> formatList = new ArrayList<String>();
        formatList.add(mActivity.getString(R.string.m_share_format_image));
        if (isShareTypeChangeSupported(intent, MIME_TYPE_VIDEO)) {
            formatList.add(mActivity.getString(R.string.m_share_format_video));
        }
        String[] formatArray = new String[formatList.size()];
        formatList.toArray(formatArray);
        return formatArray;
    }
    ...
    private final class MediaItemExporter implements Runnable {
        ...
        private boolean saveAs(final Uri uri, final String prefix, final String postfix) {
            ...
            MediaScannerConnection.scanFile(mActivity, new String[] { target.getAbsolutePath() },
    ...
    public void exportCurrentPhoto() {
        ...
        removeOldFragmentByTag(DIALOG_TAG_SELECT_SHARE_TYPE);
        final String[] shareFormats = getExportFormatStrings(item);
        final SelectDialogFragment shareTypeSelectDialog = SelectDialogFragment
        .newInstance(shareFormats, null,
                mActivity.getString(R.string.m_share_format_choose_title),
                true, 0, null);
