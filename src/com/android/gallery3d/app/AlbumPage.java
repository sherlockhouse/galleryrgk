...
import com.android.gallery3d.rgk.newui.base.RgkSlotView;
import com.android.gallery3d.rgk.newui.multiselect.MSelectSlotRender;
import com.android.gallery3d.rgk.newui.multiselect.MultSelectionManager;
...
/// @}
    ...
    private final GLView mRootPane = new GLView() {
        ...
        protected void onLayout(
            ...
            int slotViewTop = mActivity.getGalleryActionBar().getHeight() + fullscreenTop + 1;
            int slotViewBottom = bottom - top;
            slotViewBottom -= mActivity.getResources().getDimension(R.dimen.navigation_bar_height);
    ...
    private void onGetContent(final MediaItem item) {
        ...
        if (mData.getString(GalleryActivity.EXTRA_CROP) != null) {
            ...
            /// @}
            //add JLYLB-218 yejianming 20151124
    ...
    public void onLongTap(int slotIndex) {
        ...
        if (enableLongpress2Drag) {
            ((RgkSlotView) mSlotView).setmDragSelectActive(true, slotIndex);
        }
    ...
    protected void onCreate(Bundle data, Bundle restoreState) {
        ...
        initializeViews();
        initializeData(data);
        //gulincheng 20161115 BWLLYBN-361 start
        if (!mActivity.isInMultiWindowMode()) {
            Rect frame = new Rect();
            mActivity.getWindow().getDecorView()
                    .getWindowVisibleDisplayFrame(frame);
            fullscreenTop = frame.top;
        }
        //gulincheng 20161115 BWLLYBN-361 end
    ...
    private void initializeViews() {
        mSelectionManager = new MultSelectionManager(mActivity, false);
        mSelectionManager.setSelectionListener(this);
        Config.AlbumPage config = Config.AlbumPage.get(mActivity);
        /// M: [FEATURE.MODIFY] fancy layout @{
        // mSlotView = new SlotView(mActivity, config.slotViewSpec);
        mSlotView = new RgkSlotView(mActivity, config.slotViewSpec,
        ...
        mAlbumView = new MSelectSlotRender(mActivity, mSlotView,
                mSelectionManager, config.placeholderColor);
        mSlotView.setSlotRenderer(mAlbumView);
        mRootPane.addComponent(mSlotView);
        mSlotView.setListener(new RgkSlotView.RgkSimpleListener() {
            @Override
            public void selectRange(int initialSeletion,
                    int lastDraggedIndex, int minReached, int maxReached) {
                AlbumPage.this.selectRange(initialSeletion, lastDraggedIndex, minReached, maxReached);
            }
    ...
    public void onSelectionModeChange(int mode) {
        switch (mode) {
            case SelectionManager.ENTER_SELECTION_MODE: {
                ...
                enableLongpress2Drag = true;
                performHapticFeedback(HapticFeedbackConstants.LONG_PRESS);
                break;
            }
            case SelectionManager.LEAVE_SELECTION_MODE: {
                enableLongpress2Drag = false;
    ...
    protected boolean mRestoreSelectionDone;
    ...
    protected Toast mWaitToast = null;
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions,
        ...
        if (PermissionHelper.isAllPermissionsGranted(permissions, grantResults)) {
            ...
            PermissionHelper.showDeniedPromptForTecnoDismiss(mActivity);
        }
    }
    /// @}
    ...
    //add by scott
    public GLView getmRootPane() {
		return mRootPane;
	}
    
    protected boolean enableLongpress2Drag;

	public void setSelected(int slotIndex, boolean selected) {
		if (!mIsActive)
			return;
		...
		// / M: [BUG.MODIFY] @{
		// If restore selection not done in selection mode,after click one slot,
		// show 'wait' toast
		// mSelectionManager.toggle(item.getPath());
		// mSlotView.invalidate();
		if (mRestoreSelectionDone) {
			...
			// / @}
			((MultSelectionManager) mSelectionManager).setPathSelected(
					item.getPath(), selected);
			mSlotView.invalidate();
		} else {
			if (mWaitToast == null) {
				mWaitToast = Toast.makeText(mActivity,
						com.android.internal.R.string.wait, Toast.LENGTH_SHORT);
			}
			mWaitToast.show();
		}
		// / @}
	...
	protected void selectRange(int from, int to, int min, int max) {

		if (from == to) {
			Log.i("jzbg", "from = to" + " from:" + from + "  to:" + to);
