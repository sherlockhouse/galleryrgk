...
public class MenuExecutor {
    ...
    private Intent getIntentBySingleSelectedPath(String action) {
        ...
        if (android.os.SystemProperties.getInt("ro.voice_capture_support", 0) == 1) {
            if (CropActivity.CROP_ACTION.equals(action) || Intent.ACTION_EDIT.equals(action)) {
                MediaObject object = manager.getMediaObject(path);
                if (object instanceof MediaItem) {
                    MediaItem mediaItem = (MediaItem) object;
                    if (mediaItem.mHasVoice) {
                        String mimeType = getMimeType(manager.getMediaType(path));
                        Intent intent = new Intent(action).setDataAndType(manager.getContentUri(path), mimeType);
                        intent.putExtra("voiceBytes", mediaItem.mVoiceBytes);
                        return intent;
                    }
                }
            }
        }
        Log.d("sqm", "MenuExecutor---------voice photo not support-->>return.");
        //(end)
        String mimeType = getMimeType(manager.getMediaType(path));
    ...
    public void onMenuClicked(int action, ProgressListener listener,
        ...
        switch (action) {
            ...
            case R.id.action_rotate_cw:
                ...
                //20160705 gulincheng DWYSBM-351 start 
                showDialog = false;
                //20160705 gulincheng DWYSBM-351 end 
                break;
