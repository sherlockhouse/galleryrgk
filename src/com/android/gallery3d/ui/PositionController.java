...
/// @}
    ...
    public int scaleBy(float s, float focusX, float focusY) {
        ...
        //gulincheng 20160524 DWYSLMA-196 start
...
//        int y = mFilmMode ? b.mCurrentY : (int) (focusY - s * mFocusY + 0.5f);
        ...
        startAnimation(p.mCurrentX , b.mCurrentY, s, ANIM_KIND_SCALE);
        //gulincheng 20160524 DWYSLMA-196 end
        return 0;
    }

        //  -1 if the intended scale is too small for the stable range.
    public int scaleByIntex(float s, float focusX, float focusY) {
        focusX -= mViewW / 2;
        focusY -= mViewH / 2;
        Box b = mBoxes.get(0);
        Platform p = mPlatform;
...
//        int y = mFilmMode ? b.mCurrentY : (int) (focusY - s * mFocusY + 0.5f);
    ...
    */
    /**
     * Add parameter MediaData[] for plug-in.
     */
    public void moveBox(int fromIndex[], boolean hasPrev, boolean hasNext,
            boolean constrained, Size[] sizes, MediaData[] datas) {
    /// @}
    ...
    private float getMinimalScale(Box b) {
        ...
        /// M: [FEATURE.ADD] plugin @{
        s = GalleryPluginUtils.getImageOptionsPlugin().getMinScaleLimit(b.mMediaData, s);
        /// @}
