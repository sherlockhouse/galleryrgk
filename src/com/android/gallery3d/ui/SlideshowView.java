...
public class SlideshowView extends GLView {
    ...
    protected void render(GLCanvas canvas) {
        ...
        if (mPrevTexture != null && alpha != 1f) {
            ...
            /// M: [BUG.ADD] @{
            FeatureHelper.drawSlideshowDrmIcon(mContext, canvas, -mPrevTexture.getWidth() / 2,
                    -mPrevTexture.getHeight() / 2, mPrevTexture.getWidth(),
                    mPrevTexture.getHeight(), mPrevMediaItem.getMediaData(),
                    1.0f / mPrevAnimation.getCurrentScale());
            /// @}
            canvas.restore();
            /// M: [BUG.ADD] @{
        ...
        if (mCurrentTexture != null) {
            ...
            /// M: [BUG.ADD] @{
            FeatureHelper.drawSlideshowDrmIcon(mContext, canvas, -mCurrentTexture.getWidth() / 2,
                    -mCurrentTexture.getHeight() / 2, mCurrentTexture.getWidth(),
                    mCurrentTexture.getHeight(), mCurrentMediaItem.getMediaData(),
                    1.0f / mCurrentAnimation.getCurrentScale());
            /// @}
            canvas.restore();
            /// M: [BUG.ADD] @{
