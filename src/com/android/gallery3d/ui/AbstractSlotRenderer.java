...
public abstract class AbstractSlotRenderer implements SlotView.SlotRenderer {
    ...
    private final ResourceTexture mVoiceIcon;
    //(end)
    ...
    protected AbstractSlotRenderer(Context context) {
        ...
        //add by songqingming for voice photo (start)
        mVoiceIcon = new ResourceTexture(context, R.drawable.ic_voice_thumbnail);
        //(end)
    }
    ...
    //add by songqingming for voice photo (start)
    protected void drawVoiceIcon(GLCanvas canvas, int width, int height) {
        int iconSize = Math.min(width, height) / 6;
        /*mVoiceIcon.draw(canvas, (width - iconSize) / 2, (height - iconSize) / 2,
                iconSize, iconSize);*/
        mVoiceIcon.draw(canvas, 10, height - iconSize - 10,
                iconSize, iconSize);
    }
    //(end)
