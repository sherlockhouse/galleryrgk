...
import java.io.RandomAccessFile; //add by songqingming for voice photo
...
public class LocalImage extends LocalMediaItem {
    ...
    public Job<Bitmap> requestImage(int type) {
        ...
        /*return new LocalImageRequest(mApplication, mPath, dateModifiedInSec,
         type, filePath);*/
        //modify by songqingming for voice photo (start)
        /*
        return new LocalImageRequest(mApplication, mPath, dateModifiedInSec,
        ...
        */
        if (android.os.SystemProperties.getInt("ro.voice_capture_support", 0) == 1) {
            return new LocalImageRequest(mApplication, mPath, dateModifiedInSec,
                    type, filePath, mimeType, mExtItem, mMediaData, this);
        } else {
            return new LocalImageRequest(mApplication, mPath, dateModifiedInSec,
                    type, filePath, mimeType, mExtItem, mMediaData);
        }
    ...
    public static class LocalImageRequest extends ImageCacheRequest {
        ...
        private MediaItem mItem; //add by songqingming for voice photo
        ...
        LocalImageRequest(GalleryApp application, Path path, long timeModified,
                int type, String localFilePath, String mimeType, ExtItem data,
                MediaData mediaData, MediaItem item) {
            super(application, path, timeModified, type, mimeType, MediaItem
                    .getTargetSize(type));
            mLocalFilePath = localFilePath;
            mData = data;
        ...
        //add by songqingming for voice photo (start)
        @Override
        public void onVoiceParse(JobContext jc) {
            if (mItem != null && !mItem.mDecoded) {
        	try {
		    mItem.parse(mLocalFilePath);
		} catch (Exception e) {
		    e.printStackTrace();
		    mItem.mHasVoice = false;
		    mItem.mVoiceBytes = null;
		    Log.d("sqm", "LocalImage: onVoiceParse() has Exception: " + e.getMessage());
		}
            }
        }
        //(end)
    ...
    public Job<BitmapRegionDecoder> requestLargeImage() {
        ...
        if (android.os.SystemProperties.getInt("ro.voice_capture_support", 0) == 1) {
            return new LocalLargeImageRequest(filePath, this);
        } else {
            return new LocalLargeImageRequest(filePath);
        }
        //(end)
    ...
    public static class LocalLargeImageRequest
        ...
        MediaItem mItem; //add by songqingming for voice photo
        ...
        //add by songqingming for voice photo (start)
        public LocalLargeImageRequest(String localFilePath, MediaItem item) {
            mLocalFilePath = localFilePath;
            mItem = item;
        }
        //(end)
        ...
        public BitmapRegionDecoder run(JobContext jc) {
            ...
            if (mItem != null && !mItem.mDecoded) {
                try {
                    ...
                    mItem.mHasVoice = false;
                    mItem.mVoiceBytes = null;
                    Log.d("sqm", "LocalImage: LocalLargeImageRequest.run() has Exception: " + e.getMessage());
                }
            }
            //(end)
    ...
    public void parse(String filePath) throws Exception {
    	Log.d("sqm", "LocalImage[parse] filePath=" + filePath);
	...
	Log.d("sqm", "LocalImage[parse] length==" + length);
	long seek = length - 1;
	raf.seek(seek);
	byte tagLength = raf.readByte();
	Log.d("sqm", "LocalImage[parse] tagLength==" + tagLength);
	...
	if (tag.startsWith("rgkvoice_")) {
	    ...
	    Log.d("sqm", "LocalImage[parse] voiceLength==" + voiceLength);
	    byte[] voiceBytes = new byte[voiceLength];
	    seek -= voiceLength;
	    raf.seek(seek);
	    raf.read(voiceBytes, 0, voiceLength);
	    mVoiceBytes = voiceBytes;
	} else {
	    mHasVoice = false;
	    mVoiceBytes = null;
	}
	...
	Log.d("sqm", "LocalImage[parse] ...end");
    }
    //(end)
}
