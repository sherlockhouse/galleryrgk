...
/// @}
    ...
    private byte[] mVoiceBytes;
    public SaveImage(Context context, Uri sourceUri, Uri selectedImageUri,
            File destination, Bitmap previewImage, Callback callback, byte[] voiceBytes)  {
        Log.d("sqm", "SaveImage() voiceBytes="+voiceBytes);
    	mContext = context;
    	mSourceUri = sourceUri;
    	mCallback = callback;
    	mPreviewImage = previewImage;
    ...
    private void maybeWriteAudioToPhoto(File file) {
    	Log.d("sqm", "SaveImage: maybeWriteAudioToPhoto{...}---mVoiceBytes="+mVoiceBytes);
        if (mVoiceBytes != null) {
            com.android.gallery3d.voice.VoiceParser.maybeWriteAudioToPhoto(file, mVoiceBytes);
        }
/*
    	if (mVoiceBytes != null) {
    		...
    		int voiceSize = mVoiceBytes.length;
    		...
    		out.write(mVoiceBytes);
    		out.write(tagBytes);
    		out.write((tagSize & 0xFF));
    		out.flush();
    	    } catch (FileNotFoundException e) {
		e.printStackTrace();
		Log.d("sqm", "SaveImage: maybeWriteAudioToPhoto...file not found.");
	    } catch (IOException e) {
		e.printStackTrace();
		Log.d("sqm", "SaveImage: maybeWriteAudioToPhoto...IOException: " + e.getMessage());
	    } finally {
		if (out != null) {
		    try {
			out.close();
			...
			e.printStackTrace();
			Log.d("sqm", "SaveImage: maybeWriteAudioToPhoto..when close stream..IOException: " + e.getMessage());
		    }
		}
	    }
    	}
*/
    ...
    public static void saveImageCopy(ImagePreset preset, final FilterShowActivity filterShowActivity,
            File destination) {
        Uri selectedImageUri = filterShowActivity.getSelectedImageUri();
        Uri sourceImageUri = MasterImage.getImage().getUri();

        //true , 90 , 1f, true is for a copy
        Intent processIntent = ProcessingService.getSaveIntent(filterShowActivity, preset,
